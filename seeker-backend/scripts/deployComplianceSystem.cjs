...
async function main() {
  ...
  try {
    ...
    console.log("\n📋 Step 5: Creating HTS NFT Collection...");
    const createCollectionTx = await ipAssetNFT.createIPAssetNFTCollection(
      "IntellectualPropertyAssets",
      "IPNFT",
      { value: ethers.parseEther("1") } // Send 1 HBAR for HTS creation
    );
    await createCollectionTx.wait();
    console.log("✅ HTS NFT Collection created");
    ...
    console.log("\n📋 Step 6: Setting up compliance officers...");
    ...
    console.log("\n📋 Step 8: Creating initial compliance verification...");
    ...
    const deploymentInfo = {
      ...
      gasUsed: {
        complianceManager: (await complianceManager.deploymentTransaction()?.wait())?.gasUsed || "unknown",
        ipAssetNFT: (await ipAssetNFT.deploymentTransaction()?.wait())?.gasUsed || "unknown",
        ipAssetManager: (await ipAssetManager.deploymentTransaction()?.wait())?.gasUsed || "unknown"
      }
    };
    ...
    console.log(`   2. Test compliance verification`);
    console.log(`   3. Verify IP asset registration with compliance`);
    console.log(`   4. Test audit trail functionality`);
    console.log(`   5. Configure compliance officers`);
