...
async function main() {
  console.log("🔄 Transferring Ownership via Old Manager and Creating HTS Collection...");
  console.log("=====================================================================");
  ...
  const oldManagerAddress = deploymentData.redeployment?.oldAddress || "0x5f3801efa089F9ee664c2Ade045735646A2eAA64";
  ...
  try {
    ...
    if (currentOwner === newManagerAddress) {
      ...
      console.log("\n📋 Step 2: Transferring ownership via old manager...");
      
      // Get the old IPAssetManagerV2 contract
      const IPAssetManagerV2 = await ethers.getContractFactory("IPAssetManagerV2");
      const oldManager = IPAssetManagerV2.attach(oldManagerAddress);
      
      // Check if the old manager has a function to transfer ownership
      // Since the old manager doesn't have this function, we need to use the old manager's owner (deployer)
      // to transfer ownership directly
      
      console.log("📋 Attempting direct ownership transfer from deployer...");
    ...
    // If ownership transfer fails, let's try a different approach
    if (error.message.includes("OwnableUnauthorizedAccount")) {
      console.log("\n💡 Ownership transfer failed. Let's try creating HTS collection directly...");
      ...
      try {
        ...
        console.log("📋 Attempting direct HTS collection creation...");
        ...
        console.error("❌ Direct HTS creation also failed:", directError);
        throw directError;
      }
    }
