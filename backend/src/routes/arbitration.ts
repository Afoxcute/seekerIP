...
import { hcsIntegration } from '../services/hcs-integration';
...
router.post('/check-infringements', async (req, res) => {
  try {
    const result = await arbitrationService.checkAndProcessInfringements();
...
/**
 * @route POST /api/arbitration/create-hcs-topic
 * @desc Create HCS topic for an IP asset
 * @access Public
 */
router.post('/create-hcs-topic', async (req, res) => {
  try {
    const { ipAssetId } = req.body;
    ...
    const result = await arbitrationService.createHCSTopicForAsset(ipAssetId);
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to create HCS topic',
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
