...
const formatCompactNumber = (value: bigint): string => {
...
export const IPPortfolio: React.FC<IPPortfolioProps> = ({
  ...
  // Filter user's assets - show all assets when no wallet is connected
  const userAssets = userAddress 
    ? Array.from(assets.entries()).filter(([_, asset]) => 
        asset.owner.toLowerCase() === userAddress.toLowerCase()
      )
    : Array.from(assets.entries()); // Show all assets when no wallet connected

  const userLicenses = userAddress
    ? Array.from(licenses.entries()).filter(([_, license]) => 
        license.licensee.toLowerCase() === userAddress.toLowerCase()
      )
    : Array.from(licenses.entries()); // Show all licenses when no wallet connected
  ...
  return (
    <div className="ip-portfolio">
      ...
      <div className="portfolio-header">
        ...
        <div className="portfolio-stats">
          ...
          <div className="stat-card">
            <div className="stat-icon">💰</div>
            <div className="stat-content">
              <div className="stat-value" title={`${formatEther(totalRevenue)} HBAR`}>
                {formatCompactNumber(totalRevenue)} HBAR
              </div>
      ...
      <div className="portfolio-nav">
        ...
        >
          🎨 {userAddress ? 'My Assets' : 'All Assets'}
        </button>
        <button 
                  ...
                  {userAssets.slice(0, 3).map(([id, asset]) => (
                    <div key={id} className="recent-asset">
                      ...
                      <div className="recent-asset-info">
                        ...
                        <div className="asset-revenue">{formatCompactNumber(asset.totalRevenue)} HBAR</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="performance-metrics">
                  <div className="metric">
                    ...
                    <span className="metric-value">
                      {totalAssets > 0 ? formatCompactNumber(totalRevenue / BigInt(totalAssets)) : '0'} HBAR
                    </span>
                  </div>
                  <div className="metric">
        ...
        {activeTab === 'assets' && (
          <div className="assets-section">
            <div className="assets-header">
              <h3>🎨 {userAddress ? 'My IP Assets' : 'Registered IP Assets'} ({userAssets.length})</h3>
              {selectedAssets.size > 0 && userAddress && (
                <button 
              ...
              {selectedAssets.size > 0 && !userAddress && (
                <div className="wallet-required-notice">
                  <p>🔐 Connect your wallet to create portfolios</p>
                </div>
              )}
            </div>
            ...
            <div className="assets-grid">
              {userAssets.map(([id, asset]) => (
                <div 
                  key={id} 
                  className={`asset-card ${selectedAssets.has(id) ? 'selected' : ''} ${!userAddress ? 'no-wallet' : ''}`}
                  onClick={() => userAddress && toggleAssetSelection(id)}
                >
                  {userAddress && (
                  ...
                  <div className="asset-info">
                    ...
                    <div className="asset-metrics">
                      <div className="metric">
                        <span>💰 Revenue</span>
                        <span>{formatCompactNumber(asset.totalRevenue)} HBAR</span>
                      </div>
                      <div className="metric">
                        <span>🎯 Royalty</span>
            ...
            {userAssets.length === 0 && (
              <div className="empty-state">
                ...
                <h3>{userAddress ? 'No IP Assets Yet' : 'No Registered IP Assets'}</h3>
                <p>{userAddress ? 'Register your first IP asset to start building your portfolio' : 'No IP assets have been registered on the platform yet'}</p>
              </div>
            )}
          </div>
        )}
                ...
                <div className="revenue-chart">
                  {userAssets.map(([id, asset]) => (
                    <div key={id} className="revenue-bar">
                      ...
                      <div className="bar-value">{formatCompactNumber(asset.totalRevenue)} HBAR</div>
                    </div>
                  ))}
                </div>
              </div>
